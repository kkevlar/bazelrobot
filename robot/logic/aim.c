...
void aim_init(void)
{
    ...
    for (i = 0; i < AIM_LOCATION_COUNT; i++)
    {
        shoot_location_tally[i] = 0;
    }
}

#define CLEAR_ALL_IS_MINS()                  \
    for (z = 0; z < AIM_LOCATION_COUNT; z++) \
    {                                        \
        is_min[z] = 0;                       \
    }
...
aim_location_t aim_select_location()
{
    ...
    for (i = 0; i < AIM_LOCATION_COUNT; i++)
    {
        if (min_tally == shoot_location_tally[i])
        {
            count_cups++;
            is_min[i] = 1;
        }
        else if (min_tally > shoot_location_tally[i])
        {
            CLEAR_ALL_IS_MINS();
            count_cups = 1;
...
void aim_position_pre_roomba(direction_t dir_target,
                             float mm_wall,
                             float mm_target)
{
    ...
    while (1)
    {
        ...
        vec_result.speed = (int16_t)(control_treat_speed(
            (float)vec_result.speed, 255.0f, 10.0f, AIM_GENERAL_BOOST_ZONE));
...
{
    aim_position_pre_roomba(DIRECTION_ID_LEFT,
                            AIM_PRE_DESIRED_WALL_DIST_MM_L_AND_C,
                            FIELD_AIM_PRE_LEFT_FROM_LEFT);
}

void aim_pre_center(void) { aim_pre_left(); }

void aim_pre_right(void)
{
    while (echo_test_mm(direction_to_echo_pin(DIRECTION_ID_RIGHT)) > 450)
        go_right();
    aim_position_pre_roomba(DIRECTION_ID_RIGHT, AIM_PRE_DESIRED_WALL_DIST_MM_R,
...
void aim_position_final_helper(direction_t dir_target, float mm_target)
{
    ...
    while (1)
    {
        ...
        vec_result.speed = (int16_t)(control_treat_speed(
            (float)vec_result.speed, AIM_FINAL_SPEED_MAX,
            AIM_FINAL_SPEED_DEAD_ZONE, AIM_FINAL_SPEED_BOOST_ZONE));
...
{
    aim_position_final_helper(DIRECTION_ID_RIGHT,
